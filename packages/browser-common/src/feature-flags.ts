import { addEventListener, eachArray, entries, extend } from './utils/general-utils'
import type { ApiResponse, Client, SendRequestInit } from './client'
import type { Disposable } from './disposable'
import type { Extension } from './extension'
import type { FlagsResponse } from './types/feature-flags'
import type {
...
export class PostHogFeatureFlags implements Extension {
    ...
    private _dynamicProperties: Disposable | undefined
    private _crossTabPersistenceUnsubscribe: (() => void) | undefined
    ...
    private _dueRefreshIntervalMs: number | undefined
    ...
    /** Host persistence may merge flag snapshots across tabs independently of KV writes. */
    protected get _crossTabPersistence():
        | {
    ...
    protected _onConfigUpdated(): void {
        if (this._client) {
    ...
    private _finishSetup(client: Client): void {
        ...
        this._crossTabPersistenceUnsubscribe = this._crossTabPersistence?.onCrossTabFeatureFlagChange(() => {
    ...
    private _prop<Key extends keyof FeatureFlagsState>(key: Key): FeatureFlagsState[Key] | undefined {
    ...
    private _markCrossTabFeatureFlagSnapshot(
        ...
        if (!this._crossTabPersistence || !statePatch[ENABLED_FEATURE_FLAGS]) {
            return
        }
        ...
        this._crossTabPersistence.markCrossTabFeatureFlagChanges(changes)
    }
    ...
    _callFlagsEndpoint(options?: { disableFlags?: boolean }): void {
        ...
        try {
            void client
                // Preserve the legacy request's explicit undefined compression field.
    ...
    updateEarlyAccessFeatureEnrollment(key: string, isEnrolled: boolean, stage?: string): void {
        ...
        this._crossTabPersistence?.markCrossTabFeatureFlagChanges({
    ...
    _fireFeatureFlagsCallbacks(errorsLoading?: boolean): void {
        ...
        this.featureFlagEventHandlers.forEach((handler) => {
            ...
            try {
                // Legacy callbacks include this key even when there is no loading outcome.
                // oxlint-disable-next-line typescript/consistent-type-assertions
                handler(flags, flagVariants, { errorsLoading } as Parameters<FeatureFlagsCallback>[2])
            } catch (error) {
                this._logger.error('Error while running feature flags callback', error)
            }
        })
