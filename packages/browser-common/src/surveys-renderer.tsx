...
} from './surveys/survey-url-prefill'
import { getNextSurveyStep } from './surveys/survey-branching'
import { applySurveyTranslationForUser } from './surveys/survey-translations'
...
export class SurveyManager {
    private _host: SurveyRenderContext
    ...
    private _automaticDisplayDispose?: (() => void) | undefined
    ...
    constructor(posthog: SurveyRenderContext) {
        this._host = posthog
    ...
    private _onLanguageChange(): void {
        ...
        const surveys = this._host.client?.kv.get<Survey[]>(SURVEYS)
        ...
        const { shadow } = retrieveSurveyShadow(overriddenSurvey, this._host)
        render(
            <SurveyPopup
    ...
    public startAutomaticDisplay(): void {
        if (this._disposed || this._automaticDisplayDispose || this._host.config.automaticDisplay === false) return
    ...
    public handlePopoverSurvey = (
        ...
        { resumeDelayFromActivation = false }: { resumeDelayFromActivation?: boolean | undefined } = {}
    ): void => {
        if (!this._host.client?.canCapture) {
            return
        }
        ...
        const { shadow } = retrieveSurveyShadow(survey, this._host)
        ...
        const renderIfStillEligible = () => {
            if (!this._host.client?.canCapture || !this._shouldDisplaySurvey(survey)) {
        ...
        const activatedAt = resumeDelayFromActivation
            ? this._host.surveys?._surveyEventReceiver?.getActivationTimestamp?.(survey.id)
            : undefined
    ...
    private _handleWidget = (survey: Survey): void => {
        ...
        const { shadow, isNewlyCreated } = retrieveSurveyShadow(translatedSurvey, this._host)
        this._renderedTargets.set(shadow, shadow.host)
    ...
    public renderPopover = (survey: Survey): void => {
        ...
        const { survey: translatedSurvey, language: surveyLanguage } = this._translateSurveyForRendering(survey)
        const { shadow } = retrieveSurveyShadow(translatedSurvey, this._host)
        this._renderedTargets.set(shadow, shadow.host)
        render(
            <SurveyPopup
                posthog={this._host}
    ...
    public renderSurvey = (survey: Survey, selector: Element, properties?: Properties): void => {
        ...
        if (this._host.config.prefillFromUrl) {
    ...
    private _translateSurveyForRendering(survey: Survey): { survey: Survey; language: string | null } {
        ...
        return applySurveyTranslationForUser(survey, this._host)
    }
    ...
    private _warnIfFeatureFlagsDisabled(flagKey: string): void {
        if (this._loggedFeatureFlagsDisabledWarning || this._host.config.featureFlagEvaluation !== false) {
            return
        }
    ...
    private _isSurveyConditionMatched(survey: Survey): boolean {
        ...
        return (
            doesSurveyUrlMatch(survey, this._host) &&
    ...
    public checkSurveyEligibility(survey: Survey): { eligible: boolean; reason?: string | undefined } {
     ...
     */
    public checkSurveyCaptureEligibility(): { eligible: boolean; reason?: string | undefined } {
        if (!this._host.client?.canCapture) {
     ...
     */
    public checkSurveyDisplayEligibility(survey: Survey): { eligible: boolean; reason?: string | undefined } {
     ...
     */
    public checkSurveyRenderability(survey: Survey): { eligible: boolean; reason?: string | undefined } {
    ...
    private _hasActionOrEventTriggeredSurvey(survey: Survey): boolean {
        ...
        const surveysActivatedByEventsOrActions: string[] | undefined =
            this._host.surveys?._surveyEventReceiver?.getSurveys()
    ...
    public getActiveMatchingSurveys = (callback: SurveyCallback, forceReload = false): void => {
        this._host.surveys?.getSurveys((surveys) => {
    ...
    public callSurveysAndEvaluateDisplayLogic = (forceReload: boolean = false): void => {
        this.getActiveMatchingSurveys((surveys) => {
            ...
            const canCaptureResponse = this._host.client?.canCapture
    ...
    public getTestAPI() {
        return {
            ...
            sortSurveysByAppearanceDelay: this._sortSurveysByAppearanceDelay.bind(this),
...
export const renderSurveysPreview = ({
    ...
    onPreviewSubmit?: ((res: string | string[] | number | null) => void) | undefined
    onPreviewBack?: (() => void) | undefined
    posthog?: SurveyRenderContext | undefined
    positionStyles?: JSX.CSSProperties | undefined
}) => {
...
export const renderFeedbackWidgetPreview = ({
    ...
    render(
        <FeedbackWidget forceDisableHtml={forceDisableHtml} survey={survey} posthog={posthog} readOnly={true} />,
        root
    )
}

// This is the main exported function
export function generateSurveys(posthog: SurveyRenderContext, isSurveysEnabled: boolean | undefined) {
    ...
    if (posthog.config.automaticDisplay === false) {
    ...
    surveyManager.startAutomaticDisplay()
    return surveyManager
}

function startAutomaticDisplay(surveyManager: SurveyManager): () => void {
...
type UseHideSurveyOnURLChangeProps = {
    ...
    removeSurveyFromFocus?: ((survey: SurveyWithTypeAndAppearance) => void) | undefined
...
export function usePopupVisibility(
    ...
    useEffect(() => {
        ...
        const showSurvey = () => {
            ...
            if (!skipShownEvent) {
                posthog.client?.capture(SurveyEventName.SHOWN, {
...
interface SurveyPopupProps {
    ...
    onPreviewSubmit?: ((res: string | string[] | number | null) => void) | undefined
    onPreviewBack?: (() => void) | undefined
    onPopupSurveyDismissed?: (() => void) | undefined
    onCloseConfirmationMessage?: (() => void) | undefined
    ...
    isSurveyCompleted?: boolean | undefined
    /** When true, `survey shown` events will not be emitted automatically */
    skipShownEvent?: boolean | undefined
...
export function FeedbackWidget({
    ...
    const toggleSurvey = () => {
        if (!showSurvey && posthog && !readOnly && !posthog.client?.canCapture) {
            return
        }
